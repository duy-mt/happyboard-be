...
class UserService {
    ...
    static updateStatusUser = async ({ userId, status, adminId }) => {
        ...
        if (priorityAdmin > priorityTarget)
            throw new BadRequest(
                "You can't assign a status for role higher than your own.",
            )
        // block, pending, block
        if (!Object.keys(STATUS_USER).includes(status))
    ...
    static updateActivityUser = async ({ userId, isOnline }) => {
        // Check user isAdmin? if admin -> throw err
        // ...
        if (isOnline) {
    ...
    static getAllPermissons = async ({ userId }) => {
    ...
    static blockUser = async ({ userId }) => {
        ...
        console.log(
            `\x1b[31m...Action when block user ${userHolder.email}\x1b[0m`,
        )
    ...
    static getLatestUserOnline = async ({ page, limit }) => {
