...
const authentication = async (req, res, next) => {
    ...
    if (!userId)
        throw new Unauthorized(
            'Missing user ID. Please ensure the user ID is provided',
        )
    ...
    if (!accessToken)
        throw new Unauthorized(
            'Missing access token. Please ensure the access token is provided',
        )

    const isTokenExist = accessTokens.includes(accessToken)
    if (!isTokenExist)
        throw new Unauthorized(
            'Not found access token. Please login again to get new token',
        )
